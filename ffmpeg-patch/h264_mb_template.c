    ...
    if (!SIMPLE && IS_INTRA_PCM(mb_type)) {
        ...
        if ((SIMPLE || !CONFIG_GRAY || !(h->flags & AV_CODEC_FLAG_GRAY)) &&
            ...
            if (transform_bypass) {
                ...
                if (getenv("DEBUG_REAL_CHROMA") && sl->mb_x == atoi(getenv("DEBUG_REAL_CHROMA")) &&
                    sl->mb_y == (getenv("DEBUG_REAL_CHROMA2") ? atoi(getenv("DEBUG_REAL_CHROMA2")) : -999)) {
                    int16_t *base = sl->mb + (16 * 16 * 1 << PIXEL_SHIFT);
                    fprintf(stderr, "REAL CHROMA MB(%d,%d) frameno=%d: cbp=0x%x qp0=%d dc_nnz=%d raw dc(a,b,c,d)=%d %d %d %d\n",
                            sl->mb_x, sl->mb_y, g_x1900_debug_frameno, sl->cbp, qp[0],
                            sl->non_zero_count_cache[scan8[CHROMA_DC_BLOCK_INDEX + 0]],
                            base[0], base[16], base[32], base[48]);
                    fprintf(stderr, "  ac nnz(16..19): %d %d %d %d\n",
                            sl->non_zero_count_cache[scan8[16]], sl->non_zero_count_cache[scan8[17]],
                            sl->non_zero_count_cache[scan8[18]], sl->non_zero_count_cache[scan8[19]]);
                    fprintf(stderr, "  raw ac blk16: "); for (int qq=0;qq<16;qq++) fprintf(stderr, "%d ", base[qq]);
                    fprintf(stderr, "\n  raw ac blk17: "); for (int qq=0;qq<16;qq++) fprintf(stderr, "%d ", base[16+qq]);
                    fprintf(stderr, "\n  raw ac blk18: "); for (int qq=0;qq<16;qq++) fprintf(stderr, "%d ", base[32+qq]);
                    fprintf(stderr, "\n  raw ac blk19: "); for (int qq=0;qq<16;qq++) fprintf(stderr, "%d ", base[48+qq]);
                    fprintf(stderr, "\n");
                }
