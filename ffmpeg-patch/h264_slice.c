...
int ff_h264_execute_decode_slices(H264Context *h)
{
    ...
    if (context_count == 1) {
        ...
        /* Item 9 frame-scale restructure (2026-08-28): postpone deblocking
         * for this decode whenever the x1900 hook wants it (see
         * x1900_hook.h's ff_x1900_hook_installed comment) - lets our own
         * reconstruction defer/batch GPU dispatch across many rows safely,
         * since nothing gets deblocked prematurely while postponed.
         *
         * Restricted to non-I slices (2026-08-28, same session): a real,
         * hard-to-diagnose bug was found where postponing an I-slice (this
         * content's pure-intra frame 0) corrupts a small number of
         * DECLINED I4x4/I8x8 macroblocks (mb_type has MB_TYPE_INTRA4x4 set,
         * not MB_TYPE_INTRA16x16 - entirely FFmpeg's own reconstruction,
         * never touched by this project's own hook) - confirmed NOT a bug
         * in FFmpeg's postpone_filter mechanism itself (isolated re-test,
         * hook fully uninstalled, forced postponement: byte-identical
         * output) and confirmed NOT dependent on this project's own
         * top-context read change (same declined macroblocks, same
         * symptom) - genuinely some interaction between this project's own
         * hook being ACTIVE elsewhere in the same postponed slice and
         * FFmpeg's own I4x4/I8x8 reconstruction, root cause not fully
         * isolated. Restricting to non-I slices sidesteps it rather than
         * risk shipping a not-fully-understood fix: I-frames (all-intra)
         * already get zero benefit from cross-row batching anyway (the
         * left/top-neighbor-pending flush check already collapses intra
         * content to batch size 1, a pre-existing limitation, not
         * something this restructure was ever going to change) - P-slices
         * are where the real cost (GPU MC dispatch, item 10's dominant
         * finding) and thus the real benefit both live. */
        h->postpone_filter = (ff_x1900_hook_installed() &&
                               h->slice_ctx[0].slice_type_nos != AV_PICTURE_TYPE_I) ? 1 : 0;
        ...
        if (h->postpone_filter) {
            ...
            int y_end = FFMIN(sl0->mb_y + 1, h->mb_height);
            int x_end = (sl0->mb_y >= h->mb_height) ? h->mb_width : sl0->mb_x;
            h->postpone_filter = 0;
            for (j = sl0->resync_mb_y; j < y_end; j += 1 + FIELD_OR_MBAFF_PICTURE(h)) {
                sl0->mb_y = j;
                loop_filter(h, sl0, j > sl0->resync_mb_y ? 0 : sl0->resync_mb_x,
